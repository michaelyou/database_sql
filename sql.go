...
type DB struct {
	...
	freeConn []*driverConn
	// 当db.numOpen >= db.maxOpen时，排队等待创建的连接都会添加到该数据结构中
	...
	// goroutine to exit.
	// 一个goroutine会读这个channel来创建连接，当需要创建连接时就会写这个channel
	// 这里chan类型是空结构体，不占用额外的内存空间
...
// 创建连接时调用了此方法，addDepLocked(dc, dc)，两个参数都是dc，最终的map中是{dc: {dc: true}}
...
func (db *DB) startCleanerLocked() {
	// 这里通过“db.cleanerCh == nil”来保证清理清理连接的goroutine只有一个
...
// 连接池在db对象上，存储在slice中，操作时要对db对象加锁
func (db *DB) conn(ctx context.Context, strategy connReuseStrategy) (*driverConn, error) {
	...
	lifetime := db.maxLifetime // maximum amount of time a connection may be reused
	...
	if db.maxOpen > 0 && db.numOpen >= db.maxOpen {
		...
		// req等待被填充，填充完了就代表有可用的连接
		// 每当一个连接使用完毕想要释放时，通常会想到将它放入freeConn队列中
		// 这时，可以先检测connRequests中有没有坐等连接的请求，有的话就可以把连接分给那个请求，而不是放进freeConn
		// 这也符合freeConn的定义，既然有任务等着用连接，显然freeConn里是不应该有连接的
		select {
		case <-ctx.Done():
			...
			select {
			default:
			case ret, ok := <-req: // 如果此时已经给此request分配了连接，要归还
				if ok {
	...
	// 上面说了numOpen是已经建立或即将建立连接数，这里还没有建立连接，只是乐观的认为后面会成功，失败的时候再将此值减1
	// 如果等到创建完连接再把numOpen加1会导致多个协程同时创建连接时一部分会浪费，
	// 所以提前将numOpen占住，创建失败再将其减掉
	// 此操作要持有锁，而创建连接不需要锁
	db.numOpen++ // optimistically
	db.mu.Unlock()
	ci, err := db.driver.Open(db.dsn)
	...
	if err != nil {
		...
		// 一个场景：
		// 我们的maxOpen=50，当前没有创建的连接，numOpen=0，并发60，所以前50个请求将会到达上面创建连接，另外10个
		// 将会阻塞在“case ret, ok := <-req”。那么如果前50个请求创建连接都失败了呢，阻塞的10个请求将永远阻塞下去，
		// 直到有新的请求到来创建连接，等这些连接释放之后，他们才能从阻塞中恢复，这样显然是不合理的。
		// 所以maybeOpenNewConnections将通知connectionOpener根据db.connRequests长度及可创建的最大连接数重新创建连接，然后将新创建的连接发给阻塞的请求。
		// 如果maxOpen=0将不会有请求阻塞等待连接，所有请求只要从freeConn中取不到连接就会新创建。这时候db.connRequests长度为0，这个方法也就没用了
		db.maybeOpenNewConnections()
		db.mu.Unlock()
		return nil, err
	}
...
func (db *DB) QueryContext(ctx context.Context, query string, args ...interface{}) (*Rows, error) {
	var rows *Rows
	var err error
	// 首先优先使用空闲连接，如果2次取到的连接都无效则尝试新创建连接。
	for i := 0; i < maxBadConnRetries; i++ {
		rows, err = db.query(ctx, query, args, cachedOrNewConn)
		// ErrBadConn：如果连接的lifetime结束，返回此错误
		if err != driver.ErrBadConn {
			break
		}
	}
...
// 在遍历完rows之后应该要执行一下Err()
// if rows.Err() != nil {
//     return err
// }
// 防止最终不是因为EOF推出。在迭代结束后要检查错误，以确保迭代是因为数据读取完毕，而非其他“真正”错误而结束的。
// 遍历结果集的过程实际上是网络IO的过程，可能出现各种错误。健壮的程序应当考虑这些可能，而不能总是假设一切正常
